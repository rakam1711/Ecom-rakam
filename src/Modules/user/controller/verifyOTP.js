...
const verifyotp = async (req, res) => {
    try {
        ...
        if (user.wrong_attempt >= 3) {
            return new Error("you have exceed the limit of wrong attemt please resend OTP");
        }
        ...
        if (user.otp === otp || (code == otp && user.is_active === true)) {
            ...
            if (!newUser) {
                return res.status(200).json({status:true,user:"new", token:"null"})
            }
        ...
        return res.status(500).json({ status: false, message: err.message, location: "src/Modules/user/controller/verifyOTP" })
    }
}
