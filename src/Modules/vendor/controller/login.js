...
const login = async (req, res) => {
    try {
        ...
        const user = await vendorModel.findOne({ userName: body.userName });
        ...
        return res.status(200).json({ status: true, message: "login succefully", user, token })

    } catch (err) {
        return res.status(500).json({ status: false, message: err.message, location: "vendor/controller/login" })
    }
}
